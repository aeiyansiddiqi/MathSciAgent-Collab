...
from pyvis.network import Network
import pdfkit
from deepseek import DeveloperAPI

# DeepSeek API 密钥
DEEPSEEK_API_KEY = 'your_deepseek_api_key_here'

def generate(system_prompt, prompt, model='deepseek-chat', temperature=0.7, max_tokens=150, frequency_penalty=0, presence_penalty=0, top_p=1.0):
    # 设置 DeepSeek API 密钥
    api = DeveloperAPI(api_key=DEEPSEEK_API_KEY)
...
txt_content = read_text_from_pdf('lc.pdf')

# 生成图结构
graph_HTML, graph_GraphML, G, net, output_pdf = gg.make_graph_from_text(
    txt=txt_content,
    generate=generate,
    data_dir='gr/GraphReasoning/SaveFile/',
    graph_root='graph_root'
)
